...
| 敏感信息泄露 | ⚠️ | socialController.js 2处console.log输出错误信息 |
| SQL注入风险 | ✅ | ui-themes.js tag参数已添加白名单校验 |
...
| # | 严重度 | 文件 | 问题 | 建议 |
|---|--------|------|------|------|
| 1 | 🔴 | `backend/src/routes/v1/tickets.js:16,19` | `/code/:code` 和 `/number/:ticketNo` 路由未加 `auth` 中间件 | 根据业务判断：查票是否需要认证？如公开查票需防枚举 |
| 2 | 🔴 | `backend/src/utils/errorHandler.js:91-92` | 非test环境向客户端返回 `err.stack` | 生产环境绝不可返回堆栈，仅记录日志 |
| 3 | 🟡 | `backend/src/middleware/dataAdapter.js:110-111` | 返回 `error.message` + `error.stack` 给客户端 | 改为通用错误消息，堆栈仅记录服务端日志 |
| 4 | 🟡 | `backend/src/middleware/securityValidator.js:336-337,426-427` | 返回 `error.message` + `error.stack` 给客户端 | 同上，避免信息泄露 |
| 5 | 🟡 | `JujuApp_new/src/api/apiClient.ts` | Token存储在AsyncStorage（非加密存储） | 建议迁移至Keychain(iOS)/Keystore(Android) |
| 6 | 🟡 | `backend/src/controllers/socialController.js` | 2处 `console.log` 直接输出错误对象 | 改为结构化日志，避免敏感信息泄露 |
| 7 | 🟡 | `backend/src/services/bankCardService.js:179,197` | CBC模式使用环境变量IV，若IV固定则削弱安全性 | 确保每次加密使用随机IV，随密文存储 |
| 8 | 🟢 | `backend/src/middleware/auth.js:41` | 测试Token用户ID默认9999，所有测试共享身份 | 建议支持多测试用户映射 |
...
| 4 | `ui-themes.js` tag参数SQL校验 | 2026-05-03 | ✅ **已修复** — 已添加白名单校验（字母/数字/中文） |
...
| 24 | `tickets.js:81` 缺少auth中间件 | 2026-05-03 | ✅ **复查修正** — `/tickets/:id/verify` 实际已有auth |
...
#### 1. tickets.js 公开路由未认证
`backend/src/routes/v1/tickets.js:16` 和 `:19` 的 `/code/:code` 和 `/number/:ticketNo` 未加 `auth` 中间件。若票券编码可预测，存在枚举风险。

**建议**: 评估业务需求，如为公开验票需增加频率限制；如为内部查询应加认证。

#### 2. errorHandler.js 堆栈泄露
第91-92行：
```javascript
...
```
当前代码已修复为仅在development环境返回堆栈，但需确认production环境不会返回。

**建议**: 确保production环境绝不返回堆栈信息。

### 🟡 中等问题

#### 3. dataAdapter.js 错误信息泄露
第110-111行直接返回 `error.message` 和 `error.stack`，可能泄露内部路径、数据库结构等敏感信息。

#### 4. securityValidator.js 错误信息泄露
第336-337行、426-427行同样返回 `error.message` + `error.stack`。

#### 5. AsyncStorage存储Token
...
#### 6. socialController.js console.log
2处 `console.log` 直接输出错误对象，可能包含敏感信息。虽然console.log不会直接返回给客户端，但日志收集系统可能捕获并存储。

**建议**: 改为结构化日志，敏感字段脱敏。

#### 7. bankCardService.js IV使用
...
| 日志安全 | 75/100 | socialController 2处console.log需改为结构化日志 |
...
5. [ ] `socialController.js` 2处console.log改为结构化日志
