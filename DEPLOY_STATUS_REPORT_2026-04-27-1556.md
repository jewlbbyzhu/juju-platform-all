...
- 服务可能处于休眠状态（Free Tier 休眠行为）

### 子模块问题

```
git submodule status
fatal: no submodule mapping found in .gitmodules for path 'JujuApp_076'
```

项目中存在 JujuApp_076、JujuApp_fresh、JujuApp_new 等目录，但它们未在 .gitmodules 中注册为子模块，但 git 仍然认为它们是子模块。需手动清理或正确注册。
...
```
[15:56] git status - 发现未提交修改
[15:56] git add . && git commit -m "auto: pre-deploy commit 20260427155606"
[15:56] git push origin backup-auto-20260331-210742 - 成功
[15:56] curl https://juju-backend.onrender.com/health - 连接超时
```
