...
### app_download_events (10 处差异)

| 字段 | 问题类型 | 模型定义 | 数据库实际 |
|------|----------|----------|------------|
| version_id | 🔴 缺失 | INTEGER | ❌ 不存在 |
| platform | ⚠️ 可空性不匹配 | 可空 | 非空 |
| ip | 🔴 缺失 | STRING | ❌ 不存在 |
| ua | 🔴 缺失 | STRING | ❌ 不存在 |
| created_at | ⚠️ 可空性不匹配 | 非空 | 可空 |
| updated_at | ⚠️ 可空性不匹配 | 非空 | 可空 |
| version | 🟡 多余 | ❌ 未定义 | varchar |
| source | 🟡 多余 | ❌ 未定义 | varchar |
| ip_address | 🟡 多余 | ❌ 未定义 | varchar |
| user_agent | 🟡 多余 | ❌ 未定义 | varchar |
...
| 字段 | 问题类型 | 模型定义 | 数据库实际 |
|------|----------|----------|------------|
| version_name | 🔴 缺失 | STRING | ❌ 不存在 |
| platform | 🔴 缺失 | STRING | ❌ 不存在 |
| feedback_type | 🔴 缺失 | STRING | ❌ 不存在 |
| title | 🔴 缺失 | STRING | ❌ 不存在 |
| images | ⚠️ 类型不匹配 | JSONTYPE () | json () |
| status | ⚠️ 类型不匹配 | STRING () | tinyint () |
| status | ⚠️ 可空性不匹配 | 非空 | 可空 |
| status | ⚠️ 默认值不匹配 | pending | 0 |
| replied_by | 🔴 缺失 | STRING | ❌ 不存在 |
| created_at | ⚠️ 可空性不匹配 | 非空 | 可空 |
| updated_at | ⚠️ 可空性不匹配 | 非空 | 可空 |
| type | 🟡 多余 | ❌ 未定义 | varchar |
| contact | 🟡 多余 | ❌ 未定义 | varchar |
...
| source | varchar(50) | 是 | - | 下载来源 |
| ip_address | varchar(50) | 是 | - | IP地址 |
| user_agent | varchar(500) | 是 | - | User Agent |
...
| id | int(10,0) | 否 | - | - |
| user_id | int(10,0) | 否 | - | 用户ID |
| type | varchar(50) | 否 | - | 反馈类型 |
| content | text(65535) | 否 | - | 反馈内容 |
| images | json | 是 | - | 图片列表 |
| contact | varchar(100) | 是 | - | 联系方式 |
| status | tinyint(3,0) | 是 | 0 | 状态：0-待处理，1-已处理 |
| reply | text(65535) | 是 | - | 回复内容 |
