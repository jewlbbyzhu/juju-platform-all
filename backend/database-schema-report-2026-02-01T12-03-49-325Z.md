...
| 表名 | 操作建议 |
|------|----------|
| conversations | 考虑删除或创建对应模型 |
| group_members | 考虑删除或创建对应模型 |
| group_messages | 考虑删除或创建对应模型 |
| groups | 考虑删除或创建对应模型 |
| messages | 考虑删除或创建对应模型 |
| push_messages | 考虑删除或创建对应模型 |
| push_settings | 考虑删除或创建对应模型 |
...
| 字段 | 问题类型 | 模型定义 | 数据库实际 |
|------|----------|----------|------------|
| party_id | ⚠️ 可空性不匹配 | 可空 | 非空 |
| discount_amount | ⚠️ 默认值不匹配 | 0 | 0.00 |
| participant_completed | ⚠️ 可空性不匹配 | 非空 | 可空 |
| participant_completed | ⚠️ 默认值不匹配 | false | 0 |
| settlement_status | ⚠️ 可空性不匹配 | 非空 | 可空 |
...
| registration_deadline | ⚠️ 可空性不匹配 | 非空 | 可空 |
| min_price | ⚠️ 默认值不匹配 | 0 | 0.00 |
| max_price | ⚠️ 默认值不匹配 | 0 | 0.00 |
| status | ⚠️ 可空性不匹配 | 非空 | 可空 |
| draft_status | ⚠️ 可空性不匹配 | 非空 | 可空 |
...
| 字段 | 问题类型 | 模型定义 | 数据库实际 |
|------|----------|----------|------------|
| order_id | ⚠️ 可空性不匹配 | 可空 | 非空 |
| order_id | ⚠️ 默认值不匹配 | 0 | null |
