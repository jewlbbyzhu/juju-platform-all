...
class OrderService {
  async createOrder(userId, orderData) {
    try {
      ...
      return await TransactionManager.execute(async (t) => {
        ...
        // 兼容前端格式：如果前端发送的是 ticket_id + quantity，转换为 items 数组
        let orderItems = items;
        if (!items && orderData.ticket_id && orderData.quantity) {
          orderItems = [{
            ticket_type_id: orderData.ticket_id,
        ...
        return await this.getOrderById(order.id, t);
      });
    } catch (error) {
  ...
  async getOrderById(orderId, transaction = null) {
    try {
      ...
      const order = await Order.findByPk(orderId, queryOptions);
