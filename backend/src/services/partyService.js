...
class PartyService {
  ...
  async getAvailableTickets(partyId) {
    try {
      ...
      // 转换为纯JSON对象，避免返回Sequelize模型实例
      return tickets.map(ticket => {
        const plain = ticket.toJSON ? ticket.toJSON() : ticket;
        return {
          id: plain.id,
          party_id: plain.party_id,
          name: plain.name,
          description: plain.description,
          type: plain.type,
          price: plain.price,
          original_price: plain.original_price,
          available_count: plain.available_count,
          sold_count: plain.sold_count,
          max_per_user: plain.max_per_user,
          sale_start_time: plain.sale_start_time,
          sale_end_time: plain.sale_end_time,
          status: plain.status,
