...
class WebAdapter extends BaseAdapter {
  ...
  adaptParty(party) {
    ...
    return {
      ...baseParty,
      // 管理后台需要的完整聚会信息
      organizer: rawParty.user ? {
        id: rawParty.user.id,
        nickname: rawParty.user.nickname,
        avatar: rawParty.user.avatar
      } : null,
      ticketTypes: rawParty.ticket_types ? rawParty.ticket_types.map(tt => this.adaptTicketType(tt)) : [],
      // 审核信息
      auditInfo: {
        status: rawParty.audit_status || 'pending',
        auditTime: this.formatTime(rawParty.audit_time),
        auditorId: rawParty.auditor_id,
        auditNotes: rawParty.audit_notes,
        auditHistory: rawParty.audit_history ? JSON.parse(rawParty.audit_history) : []
      },
      // 财务信息
      financialInfo: {
        totalRevenue: this.formatMoney(rawParty.total_revenue || 0),
        serviceFee: this.formatMoney(rawParty.service_fee || 0),
        organizerIncome: this.formatMoney(rawParty.organizer_income || 0),
        refundAmount: this.formatMoney(rawParty.refund_amount || 0),
        settlementStatus: rawParty.settlement_status || 'pending'
      },
      // 统计数据
      analytics: {
        ...
        averageRating: rawParty.average_rating || 0,
        reviewCount: rawParty.review_count || 0
      },
      // 地理位置
      coordinates: {
        ...
        district: rawParty.district,
        city: rawParty.city,
        province: rawParty.province
      },
      // 风控信息
      riskInfo: {
        riskLevel: rawParty.risk_level || 'low',
        flaggedReasons: rawParty.flagged_reasons ? JSON.parse(rawParty.flagged_reasons) : [],
        reportCount: rawParty.report_count || 0,
        lastReportTime: this.formatTime(rawParty.last_report_time)
      },
      // 管理信息
      adminNotes: rawParty.admin_notes,
      internalNotes: rawParty.internal_notes,
      isFeatured: rawParty.is_featured || false,
      recommendWeight: rawParty.recommend_weight || 0,
      updatedAt: this.formatTime(rawParty.updated_at)
    };
  }
